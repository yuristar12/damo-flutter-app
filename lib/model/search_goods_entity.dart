...
class SearchGoodsEntity {
	...
	SearchGoodsEntity.fromJson(Map<String, dynamic> json) {
		...
		if (json['list'] != null) {
			xList = [];
      (json['list'] as List).forEach((v) { xList!.add(new ProductEntity.fromJson(v)); });
		}
		if (json['filterCategoryList'] != null) {
			filterCategoryList = [];
      (json['filterCategoryList'] as List).forEach((v) { filterCategoryList!.add(new SearchGoodsFiltercategorylist.fromJson(v)); });
		}
	}
