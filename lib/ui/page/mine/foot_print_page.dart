...
import 'package:dealful_mall/view_model/footprint_view_model.dart';
...
class _FootPrintPageState extends State<FootPrintPage> {
  ...
  _itemClick(int? goodsId) {
    NavigatorUtil.goGoodsDetails(context, goodsId.toString());
  }

  _deleteFootPrint() {
    List ids = [];
    for (FootPrintGoodsEntity footPrintGoodsEntity
        in _footPrintViewModel.goods as Iterable<FootPrintGoodsEntity>) {
      if (footPrintGoodsEntity.isCheck!) {
        ids.add(footPrintGoodsEntity.id);
      }
    }
    if (ids.length == 0) {
      XUtils.showToast(AppStrings.PLEASE_SELECT_FOOT_PRINT);
      return;
    }
    _footPrintViewModel.deleteFootPrint(ids).then((response) {
      if (response) {
        ...
        XUtils.showToast(AppStrings.DELETE_FAIL.tr);
      }
